...
class ModelActionsHandler:
    ...
    async def _on_roi_selected(
        self: "MainWindow", context_item, pdf_path: Path, bbox_norm: tuple, user_note: str
    ):
        ...
        try:
            ...
            roi_png_bytes = self.pdf_renderer.render_roi(pdf_path, bbox_norm, page_num=0, dpi=400)
            ...
            if self.r2_client:
                ...
                await self.r2_client.upload_bytes(r2_key, roi_png_bytes, content_type="image/png")
            ...
            if self.gemini_client:
                ...
                try:
                    result = await self.gemini_client.upload_file(
                        tmp_path, mime_type="image/png", display_name=f"Область: {roi_filename}"
                    )
                    ...
                    if self.chat_panel:
                        self.chat_panel.add_system_message(
                            "ROI выделен и загружен. Отправка модели...", "success"
                        )
                    ...
                    current_model = (
                        self.chat_panel.model_combo.currentData() if self.chat_panel else None
                    )
